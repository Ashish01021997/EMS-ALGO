...
max_energy_price = max(epex_spot_price)
min_energy_price = min(epex_spot_price)
P_grid_delta = 0
E_bat_delta = 0
...
for i in range(287):
        ...
        if x[0] >= x[2]:
            ...
            if x[1] == E_bat_max:
                ...
                if (x[0]) >= (x[1] - x[2] - E_bat_max):
                    ...
                    x[1] += (E_bat_delta)
                    # E_bat_loop_profile[i + 1] = E_bat_loop
                    P_grid_delta = - E_bat_delta + (x[0] - x[2])
                    bat_cycle_loop += 1
                    print("block 4")
                    counter = 4
        ...
        if x[0] <= x[2]:
            ...
            if x[1] == E_bat_min:
                ...
                if (x[0]) >= (x[2] + E_bat_min - x[1]):
                    ...
                    x[1] += (E_bat_delta)
                    # E_bat_loop_profile[i + 1] = E_bat_loop
                    P_grid_delta = (x[0] - x[2]) - E_bat_delta
                    bat_cycle_loop += 1
                    print("block 7")
                    counter = 7
                    SOC = x[1] / C_bat

        if epex_spot_price[i] < (min_energy_price + 0.1*(max_energy_price - min_energy_price)):

            P_grid_delta = 1.5*P_grid_delta

        elif epex_spot_price[i] > (min_energy_price + 0.9*(max_energy_price - min_energy_price)):

            P_grid_delta = 0.5*P_grid_delta # P_grid_delta is assumed to come from conventional power resources

        print(E_bat_delta)
        print(P_grid_delta)

        return (x[0] * epex_spot_price[i] - E_bat_delta * (bat_operational_cost) - P_grid_delta * (
                    grid_operational_cost + epex_spot_price[i]))


    def inequality_constraint1(x):

        return (x[0] - x[2] - C_bat - 1000)


    def inequality_constraint2(x):

        return (-x[0] + x[2] - C_bat - 2000)
    ...
    bounds = [(P_pv_watt * 0.5, P_pv_watt * 1.5), (E_bat_min, E_bat_max), (P_load_hour*0.5, P_load_hour*1.5)]

    # Initial guess for portfolio weights
    initial_P_pv_watt = float(P_pv_watt)
    initial_P_load_hour = float(P_load_hour)

    constraints = [{'type': 'ineq', 'fun': inequality_constraint1}, {'type': 'ineq', 'fun': inequality_constraint2},
                   {'type': 'ineq', 'fun': inequality_constraint3}, {'type': 'ineq', 'fun': inequality_constraint4}]

    initial_guess = [initial_P_pv_watt, E_bat_loop_profile[i], initial_P_load_hour]
    ...
    print(optimal_values[2] / P_load_hour)
    print(i)
